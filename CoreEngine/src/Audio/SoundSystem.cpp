...
namespace jm
{
	...
	SoundSystem::SoundSystem(unsigned int channels)
	{
		...
		JM_CORE_INFO("Found the following devices: ", alcGetString(nullptr, ALC_DEVICE_SPECIFIER));
		...
		JM_CORE_INFO("SoundSystem created with device: ", alcGetString(m_Device, ALC_DEVICE_SPECIFIER));	//Outputs used OAL device!
		...
		JM_CORE_INFO("SoundSystem has ", m_Sources.size(), " channels available!");
	}
