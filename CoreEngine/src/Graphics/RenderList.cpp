...
namespace jm
{
	...
	bool RenderList::AllocateNewRenderList(RenderList* renderlist, bool supportsTransparency)
	{
		...
		{
			JM_ERROR("Attempting to create more than 32 renderlists!", "");
			return false;
		}
...
#pragma omp parallel for
			for (auto &i : list)
			{
				i.cam_dist_sq = (i.target_obj->GetComponent<TransformComponent>()->m_WorldSpaceTransform.GetPositionVector() - m_CameraPos).LengthSquared() * mul;
			}
		};
