...
namespace Lumos
{
	namespace Graphics
	{
		GLShaderUniformDeclaration::GLShaderUniformDeclaration(Type type, const std::string& name, u32 count)
			: m_Offset(0)
			, m_Type(type)
			, m_Struct(nullptr)
			, m_Location(0)
		...
		GLShaderUniformDeclaration::GLShaderUniformDeclaration(ShaderStruct* uniformStruct, const std::string& name, u32 count)
			: m_Offset(0)
		...
		GLShaderUniformDeclaration::Type GLShaderUniformDeclaration::StringToType(const std::string& type, u32 count)
		{
			if(type == "i32")
				return Type::INT32;
			if(type == "uint")
		...
		GLShaderUniformBufferDeclaration::GLShaderUniformBufferDeclaration(const std::string& name, u32 shaderType)
			: m_Name(name)
			, m_Register(0)
			, m_Size(0)
