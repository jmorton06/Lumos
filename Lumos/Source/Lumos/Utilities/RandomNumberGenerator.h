...
namespace Lumos
{
    ...
    public:
        inline RandomNumberGenerator32()
            : generationCount(0)
        {
        }
        inline RandomNumberGenerator32(uint32_t seed)
            : engine(seed)
            , generationCount(0)
        {
        }
        inline RandomNumberGenerator32(uint32_t seed, uint64_t skip)
            : engine(seed)
            , generationCount(skip)
        {
            engine.discard(skip);
        }
    ...
    public:
        ...
        RandomNumberGenerator64(uint64_t seed)
            : engine(seed)
            , generationCount(0)
        {
        }
        RandomNumberGenerator64(uint64_t seed, uint64_t skip)
            : engine(seed)
            , generationCount(skip)
        {
            engine.discard(skip);
