...
namespace Lumos::Graphics
{
    RenderGraph::RenderGraph(uint32_t width, uint32_t height)
    {
        ...
        m_Renderer2DData.m_Limits.MaxQuads = 16; //Renderer::GetCapabilities().MaxTextureUnits;
        ...
        m_DebugDrawData.m_Renderer2DData.m_Limits.MaxQuads = 16; //Renderer::GetCapabilities().MaxTextureUnits;
    ...
    void RenderGraph::BeginScene(Scene* scene)
    {
        ...
        m_ForwardData.m_Frustum = m_Camera->GetFrustum(viewMatrix);
        {
        ...
        auto& shadowData = Application::Get().GetRenderGraph()->GetShadowData();
        {
            ...
            m_ForwardData.m_DescriptorSet[2]->SetUniform("UniformBufferLight", "viewMatrix", &viewMatrix);
    ...
    void RenderGraph::ForwardPass()
    {
        ...
        {
            Renderer::GetRenderer()->ClearRenderTarget(m_ForwardData.m_RenderTexture, commandBuffer);
        }
        else
        {
            Renderer::GetRenderer()->ClearRenderTarget(Renderer::GetMainSwapChain()->GetCurrentImage(), commandBuffer);
        }
